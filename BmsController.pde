public class BmsController {
    ...
    public int getCommand(final String str) {
        ...
        for(int i = 0; i < command.length ;i++) {
            int num = min(str.length(), command[i].length() + 1);
        ...
        while(true) {
            ...
            case 4:    // BPM
                if(str.charAt(4) == ' ' || str.charAt(4) == 0x09) {
                    // 基本コマンドなら
                    bms_header.bpm = float(content);
                    addData(BMS_TEMPO, 0, (long)bms_header.bpm);
                } else {
                    // 拡張コマンドなら
                    bms_header.bpmindex[Integer.parseInt((str.substring(4, 6)), 36)] = float(content);
                }
                break;
            case 5:    // MIDIFILE
            ...
            default:
                // 小節番号の取得（10進数）
                int line = int(str.substring(1, 4));
                // チャンネル番号の取得（16進数）
                int ch = unhex(str.substring(4, 6));
                if(ch == BMS_STRETCH) {
                    // 小節の倍率変更命令の場合
                    bms_bar[line].b_scale = float(content);
                }
                // 小節番号の最大値を記憶する
